  ...
  methods: {
    ...
    handleShareFight(where) {
      ...
      setTimeout(() => {
        ...
        if (chills.childNodes[0]) chills.removeChild(chills.childNodes[0]);
        if (chills.childNodes[0]) chills.removeChild(chills.childNodes[0]);
        if (chills.childNodes[0]) chills.removeChild(chills.childNodes[0]);
        if (chills.childNodes[0]) chills.removeChild(chills.childNodes[0]);
              ...
              switch (where) {
                case `facebook`:
                  window.open(
                    ...
                    } vs ${self.fight.target_nickname ||
                      ' The Goverment'}, ${imgurl.toLowerCase()}`,
                    'myWindow',
                ...
                case `forum`:
                  window.open(
                    `https://drugwars.tokenbb.io/new?category=all-fights&title=Check my latest fight ! ${
                      ...
                      ' The Goverment'}&content=![dw](${imgurl.toLowerCase()})`,
                    'myWindow',
                    'width=800,height=600',
                  );
                  self.share = false;
                    ...
                    const post = [
                      [
                        'comment',
                        {
                          ...
                          title: `Check my latest fight ! ${self.fight.attacker_nickname} vs ${self
                            .fight.target_nickname || ' The Goverment'}`,
                ...
                case `reddit`:
                  window.open(
                    `https://www.reddit.com/submit?url=${ref}&sr=${
                      ...
                      self.fight.attacker_nickname
                    } vs ${self.fight.target_nickname || ' The Goverment'} ${mes}`,
                    'myWindow',
                    'width=800,height=600',
                  );
                  self.share = false;
