  ...
  rawClient.subscribe((data, message) => {
    ...
    if (message[1].body === 'end_inc_attack') {
      ...
      store.dispatch('notify', {
        type: 'success',
        icon: 'person',
        message: `New troops ${message[1].body.message.unit} x${message[1].body.message.amount
          } are now available in ${message[1].body.message.territory}:${message[1].body.message.base
          }!`,
      });
